...
### Batch 3 — admin sweep + digest polish + login revisit + final Tailwind cleanup (landed)

**Scope**: Sweep the remaining hardcoded Tailwind color classes (`bg-white`, `border-gray-*`, `text-blue-*`, `text-red-*`, etc.) across the view layer. Most were concentrated in admin-org.ts (35 instances) and admin.ts (9 instances). The view layer drops to 0 raw Tailwind color classes after this batch — every surface is token-driven.

- [X] T163 **`.ga-btn-danger` token added.** New utility class in `src/views/design-tokens-css.ts` for destructive actions (deactivate location/angel/individual). Saturated `--ga-red-strong` background, white text. Used everywhere admin-org previously emitted `bg-red-600 text-white hover:bg-red-700`. **DoD**: `.ga-btn-danger` defined; admin-org delete-confirm forms render with the new class.

- [X] T164 **admin-org.ts full cream sweep.** Settings sidebar → cream surface, active item gold-wash + gold-dim text. Settings hub overview + count tiles → cream + cream-soft inner. Locations / Angels / Individuals / Managers / Shift schedules / Recipients / Ops list views → `<ul class="ga-surface-cream">` with auto-divider. All form text/select inputs → cream-soft surface with cream-dim border, gold focus ring (light-themed inputs sit on cream surfaces — the dark `.ga-input` would be jarring). Add buttons → `ga-btn ga-btn-primary`. Edit/Delete row actions → `ga-link` / `--ga-red-strong` inline. Cancel links → `ga-link`. Delete-confirm warning cards → cream + amber 4px left bar (`var(--ga-amber)`) instead of bg-white-with-amber-border. Delete-confirm danger button → `ga-btn ga-btn-danger`. Form footer dividers → `--ga-cream-dim`. **Result**: 35 hardcoded Tailwind color classes → 0. **Files**: `src/views/admin-org.ts`.

- [X] T165 **admin.ts (upload page) cream sweep.** Drag-and-drop zone → cream surface with dashed cream-dim border + on-light-subtle icon. Upload-skeleton card → cream. Fatal-error card → cream + red 4px left bar. Upload summary card → cream. Skipped-rows divider strategy → cream-dim border-bottom per row + cream-dim border-top on the list. "Back to dashboard" / "Upload another file" links → `ga-link`. **Result**: 9 hardcoded Tailwind color classes → 0. **Files**: `src/views/admin.ts`.

- [X] T166 **Final view-layer Tailwind sweep.** ui.ts breadcrumb mobile-disclosure → cream-aware hover + ga-focus. trend-chart.ts legend → gold-bright hover (was blue). dashboard.ts WoW delta colors → `--ga-red-bright` / `--ga-green-bright` via inline style instead of `text-red-700`/`text-green-700`. dashboard.ts compliance-arrow SVGs → same token color treatment. search.ts results dropdown → `<ul class="ga-surface-cream">`. **Result**: 6 additional hardcodes across ui.ts (1) + trend-chart.ts (1) + dashboard.ts (3) + search.ts (2) → 0.

- [X] T167 **Login revisit.** Verified the Batch 1.6 logo-size philosophy still composes well on the standalone login page. Login keeps its 56px centered logo (separate from the 60px header logo on authenticated views — login has no header to compete with). Gradient background reads through. Centered card balance preserved at desktop and 375px mobile. No code changes needed.

- [X] T168 **Batch 3 verification pass.** Screenshots saved to `C:/Users/DELL/AppData/Local/Temp/ga-shots-batch3/` for login (pre-auth), settings hub, all 7 admin section lists (locations / angels / individuals / managers / shift-schedules / recipients / ops), one form (location edit), one delete-confirm (location), upload page, and digest preview. Captured at desktop (1440x900) + 375px mobile. Final view-layer Tailwind-color count: **0** (the 2 remaining grep hits in `design-tokens-css.ts` are documentation comments referencing the legacy `bg-white` pattern). Typecheck clean, 139/139 tests pass.
